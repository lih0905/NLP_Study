...
def make_iter(batch_size, mode, train_data=None, valid_data=None, test_data=None):
    ...
    if mode == 'train':
        ...
        train_iter, valid_iter = ttd.BucketIterator.splits( # BucktIterator 를 쓰면 training data중 길이가 비슷한 것들끼리 묶어서 연산함 반면 Iterator는 차례대로
...
class Params:
    ...
    def load_vocab(self):
        ...
        params = {'input_dim': len(kor.vocab), 'output_dim': len(eng.vocab), # input_dim은 각각 입력에 가능한 경우의 수, out_dim도 비슷
